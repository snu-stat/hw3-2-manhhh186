...
TeamRuns <- Teams %>%
  ...
  select(yearID, teamID, lgID, WPct, run_ratio, RS, RA) %>%
  filter(
    !is.na(WPct),
    !is.na(run_ratio),
    RS > 0,
...
TeamRuns_full <- TeamRuns_full %>%
  mutate(
    pred_final = predict(final_fit, type = "response")
  )

ggplot(TeamRuns_full, aes(x = pred_final, y = WPct)) +
  geom_point(alpha = 0.5) +
  geom_abline(
    slope = 1,
    intercept = 0,
  ...
  xlab("Predicted WPct from final model") +
  ylab("Observed WPct")
```
